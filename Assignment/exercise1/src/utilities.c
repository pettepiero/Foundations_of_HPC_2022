...
void command_line_parser(int *action, int *k, int *k_boundaries, int *e, char **fname, int *n, int *s, int argc, char **argv){
    *action = 0;
    ...
    while ((c = getopt(argc, argv, optstring)) != -1) {
        ...
        *k_boundaries = *k +2; /* Adding two extra rows for periodic boundaries */
        break;
...
void set_up_map_variable(int action, int evolution, int k, unsigned char *map, int maxval, char file[]){
        /*  Determines if map has to be initialized or read from file.
            ...
            Parameters:
                ...
                k: int, size of the map
                map: unsigned char*, pointer to the map
    ...
    if(action == RUN){
        ...
        map = (unsigned char*)malloc(k*k*sizeof(unsigned char));
        if (map == NULL)
        {
